...
namespace leagueapi {
  struct TicketOfferDTO_t {
    int32_t_t count;
    uint64_t_t receivePlayerId;
    uint64_t_t offerPlayerId;
  ...
  inline void from_json(const nlohmann::json& j, TicketOfferDTO_t& v) {
    ...
    v.receivePlayerId = j.at("receivePlayerId").get<uint64_t_t>();
    v.offerPlayerId = j.at("offerPlayerId").get<uint64_t_t>();
