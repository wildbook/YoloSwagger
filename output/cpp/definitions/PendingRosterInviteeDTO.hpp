...
namespace leagueapi {
  struct PendingRosterInviteeDTO_t {
    int64_t_t inviteTime;
    uint64_t_t inviter;
    uint64_t_t inviteeId;
  ...
  inline void from_json(const nlohmann::json& j, PendingRosterInviteeDTO_t& v) {
    v.inviteTime = j.at("inviteTime").get<int64_t_t>();
    v.inviter = j.at("inviter").get<uint64_t_t>();
    v.inviteeId = j.at("inviteeId").get<uint64_t_t>();
